...
void Humanoid::Write_FINI(float right, float front, float down, float yaw)
{
...
void Humanoid::Write_FINO(float *amp, float *off)
{
...
void Humanoid::Log_Write_Attitude()
{
    //Attitude targets are all zero since Humanoid doesn't have attitude control,
    //but the rest of the log message is useful.
    ahrs.Write_Attitude(Vector3f{0,0,0});
}

// Write an EKF and POS packet
void Humanoid::Log_Write_EKF_POS()
{
    AP::ahrs().Log_Write();
}
...
UNUSED_FUNCTION
void Humanoid::Log_Write_Data(LogDataID id, int16_t value)
{
...
UNUSED_FUNCTION
void Humanoid::Log_Write_Data(LogDataID id, uint16_t value)
{
    if (should_log(MASK_LOG_ANY)) {
...
void Humanoid::Log_Write_Data(LogDataID id, int32_t value)
{
    if (should_log(MASK_LOG_ANY)) {
...
void Humanoid::Log_Write_Data(LogDataID id, uint32_t value)
{
    if (should_log(MASK_LOG_ANY)) {
...
UNUSED_FUNCTION
void Humanoid::Log_Write_Data(LogDataID id, float value)
{
    if (should_log(MASK_LOG_ANY)) {
...
void Humanoid::Log_Write_Parameter_Tuning(uint8_t param, float tuning_val, float tune_min, float tune_max)
{
...
const struct LogStructure Humanoid::log_structure[] = {
    ...
    // @URL: https://ardupilot.org/humanoid/docs/tuning.html#in-flight-tuning
    // @Field: TimeUS: Time since system startup
...
uint8_t Humanoid::get_num_log_structures() const
{
    return ARRAY_SIZE(log_structure);

}

void Humanoid::Log_Write_Vehicle_Startup_Messages()
{
