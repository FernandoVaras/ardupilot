...
class AP_Arming_Humanoid : public AP_Arming
{
public:
    friend class Humanoid;
    friend class ToyMode;

    AP_Arming_Humanoid() : AP_Arming()
    {
        // default REQUIRE parameter to 1 (Humanoid does not have an
        // actual ARMING_REQUIRE parameter)
        require.set_default((uint8_t)Required::YES_MIN_PWM);
    }

    /* Do not allow copies */
    CLASS_NO_COPY(AP_Arming_Humanoid);
