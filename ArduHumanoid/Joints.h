...
#define NUM_JOINTS 4 //Current maximum number of joints that can be added.
...
protected:
    ...
    float              _amp[NUM_JOINTS]; //amplitudes
    float              _off[NUM_JOINTS]; //offsets
    float              _freq[NUM_JOINTS]; //frequency multiplier
    float              _pos[NUM_JOINTS]; //servo positions

    float               _right_amp_factor[NUM_JOINTS];
    float               _front_amp_factor[NUM_JOINTS];
    float               _down_amp_factor[NUM_JOINTS];
    float               _yaw_amp_factor[NUM_JOINTS];

    float               _right_off_factor[NUM_JOINTS];
    float               _front_off_factor[NUM_JOINTS];
    float               _down_off_factor[NUM_JOINTS];
    float               _yaw_off_factor[NUM_JOINTS];
...
public:
    ...
    float get_throttle()
    {
        //Only for Mavlink - essentially just an indicator of how hard the joints are working.
