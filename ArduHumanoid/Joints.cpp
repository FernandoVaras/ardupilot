 ...
 */
const AP_Param::GroupInfo Joints::var_info[] = {

    // @Param: FREQ_HZ
    // @DisplayName: Joints frequency
    // @Description: This is the oscillation frequency of the joints
    // @Range: 1 10
    // @User: Standard
    AP_GROUPINFO("FREQ_HZ", 1, Joints, freq_hz, 3),
...
//constructor
Joints::Joints(uint16_t loop_rate) :
    _loop_rate(loop_rate)
{
    AP_Param::setup_object_defaults(this, var_info);
}

void Joints::setup_joints()
{
...
void Joints::add_fin(int8_t fin_num, float right_amp_fac, float front_amp_fac, float down_amp_fac, float yaw_amp_fac,
                   float right_off_fac, float front_off_fac, float down_off_fac, float yaw_off_fac)
{

    // ensure valid fin number is provided
    if (fin_num >= 0 && fin_num < NUM_JOINTS) {
...
{
    if (!_armed) {
        // set everything to zero so joints stop moving
        right_out = 0;
        front_out = 0;
        down_out  = 0;
        yaw_out   = 0;
    ...
    humanoid.Write_FINI(right_out, front_out, down_out, yaw_out);
#endif
    ...
    for (int8_t i=0; i<NUM_JOINTS; i++) {
        ...
        if (turbo_mode) {
            //double speed joints if offset at max...
...
#if HAL_LOGGING_ENABLED
    humanoid.Write_FINO(_amp, _off);
#endif
}

void Joints::output_min()
{
    right_out = 0;
    front_out = 0;
    down_out  = 0;
