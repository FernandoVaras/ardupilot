...
{
    humanoid.failsafe_check();
}

void Humanoid::init_ardupilot()
{
    ...
    allocate_motors();
    loiter = NEW_NOTHROW Loiter(humanoid.scheduler.get_loop_rate_hz());
    ...
    logger.setVehicle_Startup_Writer(FUNCTOR_BIND(&humanoid, &Humanoid::Log_Write_Vehicle_Startup_Messages, void));
#endif
...
//******************************************************************************
void Humanoid::startup_INS_ground()
{
...
bool Humanoid::position_ok() const
{
...
bool Humanoid::ekf_has_absolute_position() const
{
...
bool Humanoid::ekf_has_relative_position() const
{
...
bool Humanoid::ekf_alt_ok() const
{
    if (!ahrs.have_inertial_nav()) {
...
void Humanoid::update_auto_armed()
{
    // disarm checks
    if (ap.auto_armed) {
 ...
 */
bool Humanoid::should_log(uint32_t mask)
{
...
MAV_TYPE Humanoid::get_frame_mav_type()
{
    return MAV_TYPE_AIRSHIP;
}

// return string corresponding to frame_class
const char* Humanoid::get_frame_string()
{
...
/*
  allocate the motors class
 */
void Humanoid::allocate_motors(void)
{
    switch ((Joints::motor_frame_class)g2.frame_class.get()) {
    case Joints::MOTOR_FRAME_AIRFISH:
    default:
        motors = NEW_NOTHROW Joints(humanoid.scheduler.get_loop_rate_hz());
        break;
    }
    if (motors == nullptr) {
        AP_BoardConfig::allocation_error("FRAME_CLASS=%u", (unsigned)g2.frame_class.get());
    }
    AP_Param::load_object_from_eeprom(motors, Joints::var_info);
