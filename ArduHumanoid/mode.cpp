...
Mode::Mode(void) :
    g(humanoid.g),
    g2(humanoid.g2),
    inertial_nav(humanoid.inertial_nav),
    ahrs(humanoid.ahrs),
    motors(humanoid.motors),
    loiter(humanoid.loiter),
    channel_right(humanoid.channel_right),
    channel_front(humanoid.channel_front),
    channel_up(humanoid.channel_up),
    channel_yaw(humanoid.channel_yaw),
    G_Dt(humanoid.G_Dt)
{ };

// return the static controller object corresponding to supplied mode
Mode *Humanoid::mode_from_mode_num(const Mode::Number mode)
{
    Mode *ret = nullptr;
...
bool Humanoid::set_mode(Mode::Number mode, ModeReason reason)
{
...
bool Humanoid::set_mode(const uint8_t new_mode, const ModeReason reason)
{
    static_assert(sizeof(Mode::Number) == sizeof(new_mode), "The new mode can't be mapped to the vehicles mode number");
#ifdef DISALLOW_GCS_MODE_CHANGE_DURING_RC_FAILSAFE
    if (reason == ModeReason::GCS_COMMAND && humanoid.failsafe.radio) {
        // don't allow mode changes while in radio failsafe
        return false;
    }
#endif
    return humanoid.set_mode(static_cast<Mode::Number>(new_mode), reason);
}

// update_flight_mode - calls the appropriate attitude controllers based on flight mode
// called at 100hz or more
void Humanoid::update_flight_mode()
{
    flightmode->run();
}

// exit_mode - high level call to organise cleanup as a flight mode is exited
void Humanoid::exit_mode(Mode *&old_flightmode,
                      Mode *&new_flightmode) {}

// notify_flight_mode - sets notify object based on current flight mode.  Only used for OreoLED notify device
void Humanoid::notify_flight_mode()
{
...
void Mode::get_pilot_input(Vector3f &pilot, float &yaw)
{
    ...
    if (humanoid.failsafe.radio || !rc().has_ever_seen_rc_input()) {
        pilot.y = 0;
        pilot.x = 0;
        pilot.z = 0;
        yaw = 0;
...
{
    if (!motors->armed() || !humanoid.ap.auto_armed || humanoid.ap.land_complete) {
        return true;
    }
    return false;
}
...
{
    return humanoid.set_mode(mode, reason);
}

GCS_Humanoid &Mode::gcs()
{
    return humanoid.gcs();
}
